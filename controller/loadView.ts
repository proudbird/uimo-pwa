...
interface ViewCache {
  [viewId: string]: { code: string; map: string };
}
...
export default async function loadView(
  ...
  return new Promise(async (resolve) => {
    ...
    const viewName = viewId.split('.').slice(-1)[0];
    ...
    try {
      ...
      const result = `(() => { const module = {}; window.views["${viewId}"] = { layout: ${JSON.stringify(
        layout
      )}, data: ${JSON.stringify(data)}, module }; ((exports) => { ${
        module.code
      } })(module);})();//# sourceMappingURL=${viewId}.js.map`;

      // Cache the result and resolve the promise with the result.
      cache[viewId] = { code: result, map: module.map };
      resolve(result);
    } catch (error) {
